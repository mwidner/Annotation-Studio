...
class Annotator.Plugin.MITPermissions extends Annotator.Plugin
  ...
  updatePermissionsField: (action, field, annotation) =>
    ...
    makeOptionTag = (option) -> $(document.createElement('option')).val(option).text(option)
    options = ['Group', 'Review', 'Private']

    field = $(field).show()
    input = field.find('input').removeAttr('disabled')

    select = field.find('select').append(options.map(makeOptionTag))
